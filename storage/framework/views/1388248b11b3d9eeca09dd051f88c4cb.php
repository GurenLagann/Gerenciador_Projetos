...
<div class="space-y-8 max-w-7xl">
        ...
        <div class="rounded-2xl p-5 border" style="background:var(--surface); border-color:var(--border);">
            <div class="flex items-start justify-between mb-5">
                <div class="w-10 h-10 rounded-xl flex items-center justify-center"
                    style="background:<?php echo e($card['glow']); ?>; border:1px solid <?php echo e($card['accent']); ?>22;">
                    <svg class="w-5 h-5" style="color:<?php echo e($card['accent']); ?>;" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <?php echo $card['icon']; ?>

                    </svg>
                </div>
            </div>
            <div class="text-3xl font-bold text-white tabular-nums"><?php echo e($card['value']); ?></div>
            <div class="text-sm mt-1" style="color:var(--muted-1);"><?php echo e($card['label']); ?></div>
        </div>
        <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if ENDBLOCK]><![endif]--><?php endif; ?>
    </div>

    
    <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if($statusCounts->isNotEmpty()): ?>
    <?php $total = $statusCounts->sum(); ?>
    <div class="rounded-2xl p-5 border" style="background:var(--surface); border-color:var(--border);">
        <div class="flex items-center justify-between mb-5">
            <h3 class="text-sm font-semibold text-white">Distribuicao por Status</h3>
            <span class="text-xs" style="color:var(--muted-2);"><?php echo e($total); ?> projetos</span>
        </div>
        <div class="flex h-2 rounded-full overflow-hidden gap-px mb-5" style="background:var(--border);">
            ...
            <div class="h-full transition-all rounded-sm" style="width:<?php echo e($pct); ?>%; background:<?php echo e($statusBars[$status] ?? '#64748b'); ?>;"></div>
            <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if ENDBLOCK]><![endif]--><?php endif; ?>
        </div>
        <div class="grid grid-cols-2 md:grid-cols-3 gap-2">
            <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php $__currentLoopData = $statusCounts; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $status => $count): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
            <?php $m = $statusMeta[$status] ?? ['label'=>$status,'color'=>'#94a3b8','bg'=>'rgba(148,163,184,.1)']; ?>
            <div class="flex items-center justify-between px-3 py-2 rounded-lg" style="background:<?php echo e($m['bg']); ?>;">
                <div class="flex items-center gap-2">
                    <span class="w-1.5 h-1.5 rounded-full flex-shrink-0" style="background:<?php echo e($m['color']); ?>;"></span>
                    <span class="text-xs font-medium" style="color:<?php echo e($m['color']); ?>;"><?php echo e($m['label']); ?></span>
                </div>
    ...
    <div>
        <div class="flex items-center justify-between mb-4">
            ...
            <a href="<?php echo e(route('projects.index')); ?>" class="text-xs font-medium hover:opacity-80 transition-opacity" style="color:#818cf8;">
                Ver todos &rarr;
            </a>
        </div>
        ...
        <div class="rounded-2xl border border-dashed py-14 text-center" style="border-color:var(--border-2);">
            ...
            <form method="POST" action="<?php echo e(route('projects.scan')); ?>" class="inline">
                <?php echo csrf_field(); ?>
    ...
    <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if($recentIdeas->isNotEmpty()): ?>
    <div>
        <div class="flex items-center justify-between mb-4">
            <h3 class="text-base font-semibold text-white">Ideias Recentes</h3>
            <a href="<?php echo e(route('ideas.index')); ?>" class="text-xs font-medium hover:opacity-80 transition-opacity" style="color:#818cf8;">
                Ver todas &rarr;
            </a>
        </div>
        <div class="rounded-2xl border overflow-hidden" style="background:var(--surface); border-color:var(--border);">
            <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php $__currentLoopData = $recentIdeas; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $i => $idea): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
            <?php
            ...
            <a href="<?php echo e(route('ideas.show', $idea)); ?>"
                class="flex items-center gap-4 px-5 py-3.5 hover:bg-white/[.025] transition-colors <?php echo e($i > 0 ? 'border-t' : ''); ?>"
                style="<?php echo e($i > 0 ? 'border-color:var(--border);' : ''); ?>">
                <div class="w-2 h-2 rounded-full flex-shrink-0" style="background:<?php echo e($pm['dot']); ?>;"></div>
                <div class="flex-1 min-w-0">
                    <span class="text-sm font-medium text-white"><?php echo e($idea->title); ?></span>
                    ...
                    <span class="text-xs ml-2" style="color:var(--muted-1);"><?php echo e(Str::limit($idea->description, 60)); ?></span>
                    <?php endif; ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if ENDBLOCK]><![endif]--><?php endif; ?>
                </div>
