<div class="space-y-4">
    ...
    <div class="flex items-center justify-between">
        ...
        <button wire:click="$set('showForm', true)"
            class="outline-trigger flex items-center gap-1.5 text-xs px-3 py-1.5 rounded-lg border font-medium"
...
**negrito**, *itálico*, `código`, ## título"
            ...
            <div x-show="tab==='preview'" x-cloak
                ...
                @else
                    <p style="color:var(--muted-1); font-style:italic;">Nada para visualizar ainda...</p>
                @endif
            </div>
            ...
            <div class="flex items-center gap-2 pt-1">
                ...
                @if($editingId)
                <span class="text-xs ml-auto" style="color:var(--muted-3);">Editando anotação</span>
                @endif
            </div>
        </div>
    </div>
    ...
    <div class="space-y-3">
            ...
            <div class="p-4">
                <div class="flex items-start justify-between gap-3 mb-3">
                    <div class="flex items-center gap-2 min-w-0">
                        ...
                        @endif
                        @if($annotation->title)
                        <h4 class="text-sm font-semibold text-white truncate">{{ $annotation->title }}</h4>
                        @endif
                    </div>
                    <div class="flex items-center gap-0.5 flex-shrink-0">
                        <button wire:click="pin({{ $annotation->id }})"
                            class="icon-action p-1.5 rounded-lg text-xs"
                            style="{{ $annotation->pinned ? 'color:#818cf8; background:rgba(99,102,241,.15);' : '' }}"
                            title="{{ $annotation->pinned ? 'Desafixar' : 'Fixar' }}"
                            aria-label="{{ $annotation->pinned ? 'Desafixar anotação' : 'Fixar anotação' }}">
                            <svg class="w-3.5 h-3.5" fill="currentColor" viewBox="0 0 16 16">
                                <path d="M4.146.146A.5.5 0 0 1 4.5 0h7a.5.5 0 0 1 .5.5c0 .68-.342 1.174-.646 1.479-.126.125-.25.224-.354.298v4.431l.078.048c.203.127.476.314.751.555C12.36 7.775 13 8.527 13 9.5a.5.5 0 0 1-.5.5h-4v4.5c0 .276-.224 1.5-.5 1.5s-.5-1.224-.5-1.5V10h-4a.5.5 0 0 1-.5-.5c0-.973.64-1.725 1.17-2.189A5.921 5.921 0 0 1 5 6.708V2.277a2.77 2.77 0 0 1-.354-.298C4.342 1.674 4 1.179 4 .5a.5.5 0 0 1 .146-.354z"/>
                            </svg>
                        </button>
                        <button wire:click="edit({{ $annotation->id }})"
                            class="icon-action p-1.5 rounded-lg"
                            title="Editar" aria-label="Editar anotação">
                            <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/>
                            </svg>
                        </button>
                        <button wire:click="delete({{ $annotation->id }})"
                            wire:confirm="Excluir esta anotação? Essa ação não pode ser desfeita."
                            class="icon-action-danger p-1.5 rounded-lg"
                ...
                <div class="flex items-center justify-between mt-3 pt-3 border-t" style="border-color:var(--border);">
                    <span class="text-xs" style="color:var(--muted-3);">{{ $annotation->created_at->diffForHumans() }}</span>
                    @if($annotation->pinned)
        ...
        <div class="rounded-2xl border border-dashed py-14 text-center" style="border-color:var(--border-2);">
            ...
            <p class="text-sm font-medium text-white mb-1">Nenhuma anotação ainda</p>
            <p class="text-xs" style="color:var(--muted-2);">Clique em "Adicionar Nota" para começar a escrever.</p>
        </div>
        @endforelse
    </div>
